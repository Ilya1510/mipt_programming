...
namespace numbers {
    ...
    private:
        ...
        BigInteger& unsignedAdd(const BigInteger& other) {
            long long carry = 0;
        ...
        BigInteger& unsignedSub(const BigInteger& other) {
            long long carry = 0;
            ...
            for (int i = int(digits_.size()) - 1; i >= 0 && digits_[i] == 0; --i)
                digits_.pop_back();
            return *this;
        }

        BigInteger& unsignedSubtractRev(const BigInteger& other) {
            long long carry = 0;
    ...
    public:
        ...
        BigInteger(const std::string& str) {
            ...
            getSign() = str[0] == '-' ? -1 : 1;
            long long carry = 0;
            for (int i = int(str.size()) - 1; i >= (getSign() == -1); i -= BASE_LEN) {
                carry = 0;
                for (int j = max((long long)(getSign() == -1), i - BASE_LEN + 1); j <= i; ++j) {
                    carry *= 10;
                    carry += str[j] - '0';
                }
        ...
        string toString() const {
            ...
            for (int i = int(digits_.size()) - 1; i >= 0; --i) {
                ...
                for (int j = int(tmp.size()) - 1; j >= 0; --j) {
                    if (i == int(digits_.size()) - 1 && tmp[j] == '0' && !flag)
                        continue;
                    flag = 1;
                    res += tmp[j];
                }
        ...
        int operator[] (int x) const {
            return x >= 0 && x < (int)digits_.size() ? digits_[x] : 0;
        }
        ...
        bool operator<(const BigInteger& other) const {
            ...
            if (digits_.size() != other.digits_.size())
                ans = (digits_.size() < other.digits_.size()) ^ (getSign() == -1);
            else {
                for (int i = int(digits_.size()) - 1; i >= 0; --i) {
                    if (digits_[i] != other.digits_[i]) {
                        ans = (digits_[i] < other.digits_[i]) ^ (getSign() == -1);
                        break;
                    }
                }
            }
        ...
        BigInteger& operator+=(const BigInteger& other) {
            if (getSign() == 1 && other.getSign() == 1) {
                unsignedAdd(other);
            }
            else if (getSign() == -1 && other.getSign() == -1) {
                unsignedAdd(other);
            }
            else if (getSign() == -1 && other.getSign() == 1) {
                if (-*this <= other)
                    unsignedSubtractRev(other), changeSign();
                else
                    unsignedSub(other);
            }
            else {
        ...
        const BigInteger operator*(const BigInteger& other) const {
            ...
            ans.getSign() = getSign() * other.getSign();
            return ans;
        }
